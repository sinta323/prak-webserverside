...
$name = filter_input(INPUT_POST, 'name', FILTER_SANITIZE_STRING);//karena name itu string
$inputs['name'] = $name;
if ($name) {
    //trim untuk memotong spasi didepan atau dibelakang
 $name = trim($name);
 if ($name === '') {
...
$email = filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL);//untuk mensanitize atau memvalidasi email
$inputs['email'] = $email;
if ($email) {
 // validate email
